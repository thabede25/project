...
# Data dictionaries (embedded as per request)
users_data = {
...
# Save functions (for persisting changes)
def save_users(df):
    try:
...
class CriticalMineralsApp:
    ...
    def show_admin_menu(self):
        ...
        tk.Button(self.root, text="4. View Geographical Data", command=self.view_geo_data, font=("Arial", 12), bg="lightblue").pack(pady=5, fill="x")
    ...
    def show_investor_menu(self):
        ...
        tk.Button(self.root, text="4. View Geographical Data", command=self.view_geo_data, font=("Arial", 12), bg="lightgreen").pack(pady=5, fill="x")
    ...
    def show_researcher_menu(self):
        ...
        tk.Button(self.root, text="4. View Geographical Data", command=self.view_geo_data, font=("Arial", 12), bg="lightyellow").pack(pady=5, fill="x")
    ...
    def view_geo_data(self):
        self.clear_screen()
        tk.Label(self.root, text="Geographical Data", font=("Arial", 14), fg="blue").pack(pady=10)
        tk.Label(self.root, text="Select Site:", font=("Arial", 12)).pack()
        site = ttk.Combobox(self.root, values=self.sites_df['SiteName'].tolist(), font=("Arial", 12))
        site.pack(pady=5)
        text = tk.Text(self.root, height=15, width=80, font=("Arial", 10))
        text.pack(pady=5)
        tk.Button(self.root, text="View", command=lambda: self.display_geo_data(site.get(), text), font=("Arial", 12), bg="green", fg="white").pack(pady=5)
        tk.Button(self.root, text="Back", command=self.show_menu_by_role, font=("Arial", 12), bg="grey", fg="white").pack(pady=5)

    def display_geo_data(self, site_name, text):
        text.delete(1.0, tk.END)
        site_row = self.sites_df[self.sites_df['SiteName'].str.lower() == site_name.lower()]
        if not site_row.empty:
            mineral_id = site_row['MineralID'].iloc[0]
            country_id = site_row['CountryID'].iloc[0]
            mineral = self.mineral_df[self.mineral_df['MineralID'] == mineral_id]['MineralName'].iloc[0]
            country = self.country_df[self.country_df['CountryID'] == country_id]['CountryName'].iloc[0]
            text.insert(tk.END, f"{site_name} Details:\n")
            text.insert(tk.END, f"Location: ({site_row['Latitude'].iloc[0]}, {site_row['Longitude'].iloc[0]})\n")
            text.insert(tk.END, f"Country: {country}\n")
            text.insert(tk.END, f"Mineral: {mineral}\n")
            text.insert(tk.END, f"Production: {site_row['Production_tonnes'].iloc[0]} tonnes\n")
        else:
